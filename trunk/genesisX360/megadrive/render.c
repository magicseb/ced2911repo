...
//#define xoffset 0x50
#define xoffset 0x20 //la megadrive à un decalage de 20 sur son image
    if(bitmap.remap)
    {
        int width = (reg[12] & 1) ? 320 : 256;
	void *out = (void *)&bitmap.data[(line * bitmap.pitch)];// on s'enfou de l'offset du viewport ....
        switch(bitmap.depth)
        {
            case 8:
                remap_8(lb+xoffset, (uint8 *)out, pixel_8, width);
                break;
            case 15:
                remap_16(lb+xoffset, (uint16 *)out, pixel_15, width);
                break;
            case 16:
                remap_16(lb+xoffset, (uint16 *)out, pixel_16, width);
                break;
            case 32:
                remap_32(lb+xoffset, (uint32 *)out, pixel_32, width);
                break;
        }
    }
}
